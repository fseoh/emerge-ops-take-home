...
**To run it:** open the live dashboard at https://fseoh.github.io/emerge-ops-take-home/Dashboard/ (or `Dashboard/index.html` locally). It has today's outreach list, the lesson-5 metric by month, and the pilot's decision rule. Draft texts for the two touches are in `outbox/`. Nothing is sent from either.
